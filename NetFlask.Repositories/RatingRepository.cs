...
namespace NetFlask.Repositories
{
    public class RatingRepository : BaseRepository<RatingEntity>, IConcreteRepository<RatingEntity>
    {
        ...
        {
            string requete = @" SELECT IdMovie, IdUser, Score, DateRating FROM Rating WHERE IdMovie=" + idMovie;
            return base.Get(requete); 
        }

        public List<RatingEntity> GetByUser(int idUser)
        {
            string requete = @" SELECT IdMovie, IdUser, Score, DateRating FROM Rating WHERE IdUser=" + idUser;
            return base.Get(requete);
        }
